...
def lambda_handler(event, context):
    ...
    if "AWSCURRENT" in versions[token]:
        logger.info("Secret version %s already set as AWSCURRENT for secret and Slack the update %s." % (token, arn))
        return
    elif "AWSPENDING" not in versions[token]:
        logger.error("Secret version %s not set as AWSPENDING for rotation of secret and  Slack the update %s." % (token, arn))
...
def create_secret(service_client, arn, token):
    ...
    try:
        ...
        current_dict['username'] = current_dict['username']

        # Generate a random password
        passwd = service_client.get_random_password(ExcludePunctuation=True,PasswordLength=16)
...
def finish_secret(service_client, arn, token):
    ...
    for version in metadata["VersionIdsToStages"]:
        if "AWSCURRENT" in metadata["VersionIdsToStages"][version]:
            if version == token:
                ...
                logger.info("finishSecret: Version %s already marked as AWSCURRENT for and Message: Successfully sent message to slack %s" % (version, arn))
                return
    ...
    slack_message(service_client, arn, metadata['Name'])
    ...
    try:
        if use_ssl:
            ...
            conn = psycopg2.connect(host=secret_dict['host'], user=secret_dict['username'], password=secret_dict['password'], database=dbname, port=port,
                                connect_timeout=5, sslrootcert='/etc/pki/tls/cert.pem')
        else:
            conn = psycopg2.connect(host=secret_dict['host'], user=secret_dict['username'], password=secret_dict['password'], database=dbname, port=port,
...
def slack_message(service_client, arn, metadata, **kwargs):
    """Slack message to post to Slack"""
    webhook_url = os.environ['SLACK_WEBHOOK_ENDPOINT']
    current_user = get_secret_dict(service_client, arn, "AWSCURRENT")['username']
    for k, v in kwargs.items():
        logger.info("Setting %s to %s" % (k, v))
    message_body = "Secret for db user {} has been rotated Successfully on {}".format(current_user,metadata)
